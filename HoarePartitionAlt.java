 ...
 */
public class HoarePartitionAlt implements PartitionAlgorithm {
   ...
   */
  public static final PartitionAlgorithm ALGORITHM = new HoarePartitionAlt();
  ...
  public int partition(IntArray iarr, int lb, int ub) {
    ...
    int right = 0;
    while (small < large) {
      if ((left = iarr.get(small)) <= pivot) {
        ++small;
      } else if ((right = iarr.get(large - 1)) > pivot) {
        --large;
      } else {
